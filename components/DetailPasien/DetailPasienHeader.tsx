...
import type { DetailPasienProps } from "../../types/detailPatient";

interface Props extends DetailPasienProps {
  systemTime: string;
}

const DetailPasienHeader: React.FC<Props> = ({ systemTime, noRm, tanggal, hariPerawatanKe }) => {
  return (
      ...
      <div className="relative flex flex-col gap-4 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative space-y-2">
          ...
          <h2 className="flex items-center gap-2 text-lg font-semibold text-emerald-50 sm:text-xl">
            ...
            Monitoring Detail Pasien
          </h2>
          <p className="max-w-xl text-xs text-emerald-100/90">
            Input data respirasi, neuro, cairan masuk-keluar, masalah dan tindakan per jam.
          </p>
        </div>
        ...
        <div className="relative flex flex-col items-end gap-1 text-[11px] text-emerald-100/90">
          <div className="inline-flex items-center gap-2 rounded-full bg-emerald-800/70 px-3 py-1 ring-1 ring-emerald-500/40">
            ...
            <span>
              Jam sistem: <span className="font-mono font-semibold text-emerald-50">{systemTime}</span>
            </span>
          </div>
          {noRm && (
            <div className="flex items-center gap-1">
              ...
              <span>
                No. RM: <span className="font-semibold text-emerald-50">{noRm}</span>
              </span>
            </div>
          )}
          {tanggal && (
            <div className="flex items-center gap-1">
              ...
              <span>
                Tanggal: <span className="font-semibold text-emerald-50">{tanggal}</span>
              </span>
            </div>
          )}
          {typeof hariPerawatanKe !== "undefined" && (
            <span>
              Hari perawatan ke: <span className="font-semibold text-emerald-50">{hariPerawatanKe}</span>
            </span>
          )}
        </div>
      </div>
...
export default DetailPasienHeader;
