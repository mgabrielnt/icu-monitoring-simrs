...
interface InstruksiObatHeaderProps {
  hariPerawatan: number;
  instruksi: InstruksiObatItem[];
}

export default function InstruksiObatHeader({ hariPerawatan, instruksi }: InstruksiObatHeaderProps) {
  
  // Ambil data dari instruksi pertama (jika ada)
  const first = instruksi[0] || null;

  const dokter = first?.namaDokter || "-";
  const tglMulai = first?.tglMulai || "-";
  const tglStop = first?.tglStop || "-";
      ...
      <div className="relative flex flex-col gap-4 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div className="flex h-16 w-16 items-center justify-center rounded-full bg-emerald-700/50 ring-2 ring-emerald-500/40 backdrop-blur-sm">
            <ClipboardList className="h-8 w-8 text-emerald-100" />
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1 text-sm text-emerald-100/90">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>
                  Jumlah Instruksi:{" "}
                  <b className="text-emerald-50">{instruksi.length}</b>
                </span>
              </div>

              <div className="flex items-center gap-2">
                <ArrowLeft className="h-4 w-4 opacity-0" /> {/* For alignment */}
                <span>
                  Periode:{" "}
                  <b className="text-emerald-50">{tglMulai}</b> –{" "}
                  <b className="text-emerald-50">{tglStop}</b>
                </span>
              </div>
            </div>
